...
{   // This is from the textbook
	...
	for (int unsorted = 1; unsorted < size; unsorted++)
	{
		...
		while (location > 0 && comp(nextItem, s[location - 1]))
		{
...
int main()
{
	...
	// destruction time.
	{
		...
		vector<Sensor*> sensorPtrs(auxSensors.size());
		vector<Sensor*>::iterator i = sensorPtrs.begin();
		vector<Sensor>::iterator auxIt = auxSensors.begin();
		
		while (i != sensorPtrs.end() && auxIt != auxSensors.end())
		{
			(*i) = &(*auxIt);
			i++;
			auxIt++;
		...
		it = sensors.begin();
		vector<Sensor*>::iterator ptrIt = sensorPtrs.begin();
		while (it != sensors.end() && ptrIt != sensorPtrs.end())
		{
			*it = *(*ptrIt);
			it++;
			ptrIt++;
